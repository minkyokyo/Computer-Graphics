...
const myLight = new THREE.DirectionalLight(0xff00ff, 0.5);
...
renderer.setAnimationLoop(() => {
  //----------------- cube ModelMatrix -----------------
  let cubeMatrix = cube.matrixWorld.clone();
  //  큐브 모델 매트릭스를 GPU에 보내자~
  cube.material.uniforms.modelMatrix.value = cubeMatrix.clone();

  //----------------- 노말 트랜스폼 매트릭스를 구하자. -----------------
  // 1. 모델 매트릭스에 역행렬 취하고  2. 트랜스포즈 하기
  let normalMatrix = cubeMatrix.clone().invert().transpose();
  // 노말 변환 매트릭스를 GPU에 보내자~
  cube.material.uniforms.normalMatrix.value = normalMatrix.clone();

  //----------------- 카메라 위치를 구하자. -----------------
  let cameraPosition = new THREE.Vector3();
  // ----------------- 카메라의 월드 스페이스의 위치 구하기-----------------
  camera.getWorldPosition(cameraPosition);
  // ----------------- 카메라 월드 상 위치를 GPU에 보내자~
  cube.material.uniforms.cameraPosition.value = cameraPosition.clone();
